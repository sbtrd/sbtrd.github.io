/* Conduct'Home v1.86 — tableau de bord et calendrier enrichis. */
(function (global) {
'use strict';
const modules = {
...
function buildProjectStagesFromModel(modelId, models, startDate) {
    ...
    const configured = model.stages.slice().sort((a,b) => Number(a.order||0)-Number(b.order||0));
    const byId = new Map(configured.map((item) => [item.stageId, item]));
    return stages_1.STAGES.map((stage) => {
        ...
        const weeks = Number((config.durationWeeks ?? (Number(config.durationDays || 0) / 5)) || 0);
        cursor = addWorkingDays(cursor, Math.max(0, Math.round(weeks * 5)));
        return { stageId: stage.id, status: 'todo', plannedDate, note: config.note || undefined, modelColor: config.color || undefined, modelDurationWeeks: weeks, modelDurationDays: Math.max(0, Math.round(weeks * 5)) };
    });
}
function ModelsView({ models, onChange }) {
    const [selectedId, setSelectedId] = (0, react_1.useState)(models[0]?.id || '');
    const [draggedStageId, setDraggedStageId] = (0, react_1.useState)('');
    const selected = models.find((m) => m.id === selectedId);
    const create = () => { const model={id:`m-${crypto.randomUUID()}`,name:'Nouveau modèle',stages:[]}; onChange([...models,model]); setSelectedId(model.id); };
    const updateModel = (patch) => onChange(models.map((m)=>m.id===selectedId?{...m,...patch}:m));
    const addStage = (stageId) => { if(!selected||selected.stages.some(s=>s.stageId===stageId))return; updateModel({stages:[...selected.stages,{stageId,order:selected.stages.length+1,durationWeeks:1,color:'#8b1d2c'}]}); };
    const removeStage = (stageId) => updateModel({stages:selected.stages.filter(s=>s.stageId!==stageId).map((s,i)=>({...s,order:i+1}))});
    const updateStage = (stageId,patch) => updateModel({stages:selected.stages.map(s=>s.stageId===stageId?{...s,...patch}:s)});
    const moveStage = (stageId,targetId) => { const list=selected.stages.slice().sort((a,b)=>a.order-b.order); const from=list.findIndex(s=>s.stageId===stageId), to=list.findIndex(s=>s.stageId===targetId); if(from<0||to<0||from===to)return; const [item]=list.splice(from,1); list.splice(to,0,item); updateModel({stages:list.map((s,i)=>({...s,order:i+1}))}); };
    const removeModel = () => { const next=models.filter(m=>m.id!==selectedId); onChange(next); setSelectedId(next[0]?.id||''); };
    const available=stages_1.STAGES.filter(s=>!selected?.stages.some(i=>i.stageId===s.id));
    const sourceItems=available.map((stage)=>(0,jsx_runtime_1.jsxs)("div",{className:"model-source-item",draggable:true,onDragStart:()=>setDraggedStageId(stage.id),onDragEnd:()=>setDraggedStageId(''),children:[(0,jsx_runtime_1.jsx)("span",{className:"drag-handle",children:"⠿"}),(0,jsx_runtime_1.jsx)("span",{className:"model-source-color",style:{background:stage.color||'#8b1d2c'}}),(0,jsx_runtime_1.jsx)("span",{children:stage.label})]},stage.id));
    const rows=selected?selected.stages.slice().sort((a,b)=>a.order-b.order).map((item,index)=>{ const def=stages_1.STAGES.find(s=>s.id===item.stageId); return (0,jsx_runtime_1.jsxs)("div",{className:"model-drop-item",onDragOver:e=>e.preventDefault(),onDrop:e=>{e.stopPropagation(); if(draggedStageId&&selected.stages.some(s=>s.stageId===draggedStageId))moveStage(draggedStageId,item.stageId);},children:[(0,jsx_runtime_1.jsx)("span",{className:"drag-handle",draggable:true,onDragStart:()=>setDraggedStageId(item.stageId),onDragEnd:()=>setDraggedStageId(''),children:"⠿"}),(0,jsx_runtime_1.jsx)("b",{className:"model-order",children:index+1}),(0,jsx_runtime_1.jsxs)("div",{className:"model-drop-name",children:[(0,jsx_runtime_1.jsx)("i",{style:{background:item.color||'#8b1d2c'}}),(0,jsx_runtime_1.jsx)("strong",{children:def?.label||item.stageId})]}),(0,jsx_runtime_1.jsxs)("label",{className:"model-week-input",children:[(0,jsx_runtime_1.jsx)("span",{children:"Durée"}),(0,jsx_runtime_1.jsx)("input",{type:"number",min:"0.2",step:"0.1",value:item.durationWeeks??(Number(item.durationDays||0)/5),onChange:e=>updateStage(item.stageId,{durationWeeks:Number(e.target.value)})}),(0,jsx_runtime_1.jsx)("em",{children:"sem."})]}),(0,jsx_runtime_1.jsx)("input",{type:"color",className:"model-color-picker",value:item.color||'#8b1d2c',onChange:e=>updateStage(item.stageId,{color:e.target.value})}),(0,jsx_runtime_1.jsx)("button",{className:"model-remove-stage",onClick:()=>removeStage(item.stageId),title:"Retirer l'étape",children:"×"})]},item.stageId); }):[];
    const modelEditor = selected ? (0,jsx_runtime_1.jsxs)("main",{className:"model-builder",children:[
        (0,jsx_runtime_1.jsxs)("div",{className:"model-builder-top panel",children:[(0,jsx_runtime_1.jsxs)("label",{children:[(0,jsx_runtime_1.jsx)("span",{children:"Nom du modèle"}),(0,jsx_runtime_1.jsx)("input",{value:selected.name,onChange:e=>updateModel({name:e.target.value})})]}),(0,jsx_runtime_1.jsx)("button",{className:"secondary-button",onClick:removeModel,children:"Supprimer le modèle"})]}),
        (0,jsx_runtime_1.jsxs)("div",{className:"model-dnd-layout",children:[
            (0,jsx_runtime_1.jsxs)("section",{className:"model-source panel",children:[(0,jsx_runtime_1.jsxs)("div",{className:"model-section-title",children:[(0,jsx_runtime_1.jsx)("strong",{children:"Étapes disponibles"}),(0,jsx_runtime_1.jsx)("span",{children:"Glisser-déposer"})]}),...sourceItems,sourceItems.length===0&&(0,jsx_runtime_1.jsx)("p",{className:"model-empty",children:"Toutes les étapes sont déjà utilisées."})]}),
            (0,jsx_runtime_1.jsxs)("section",{className:"model-dropzone panel",onDragOver:e=>e.preventDefault(),onDrop:e=>{e.preventDefault(); if(draggedStageId&&!selected.stages.some(s=>s.stageId===draggedStageId))addStage(draggedStageId); setDraggedStageId('');},children:[(0,jsx_runtime_1.jsxs)("div",{className:"model-section-title",children:[(0,jsx_runtime_1.jsx)("strong",{children:"Planning du modèle"}),(0,jsx_runtime_1.jsx)("span",{children:`${selected.stages.length} étape${selected.stages.length>1?'s':''}`})]}),...rows,rows.length===0&&(0,jsx_runtime_1.jsxs)("div",{className:"model-drop-empty",children:[(0,jsx_runtime_1.jsx)("strong",{children:"Glisse ici les étapes de ton modèle"}),(0,jsx_runtime_1.jsx)("span",{children:"Tu pourras ensuite régler leur ordre, leur durée et leur couleur."})]})]})
        ]})
    ]}):null;
    return (0,jsx_runtime_1.jsxs)("section",{className:"models-page models-page-dnd",children:[
        (0,jsx_runtime_1.jsxs)("header",{className:"models-header",children:[(0,jsx_runtime_1.jsxs)("div",{children:[(0,jsx_runtime_1.jsx)("span",{className:"eyebrow",children:"AUTOMATISATION DU PLANNING"}),(0,jsx_runtime_1.jsx)("h1",{children:"Modèles de chantier"}),(0,jsx_runtime_1.jsx)("p",{children:"Glisse les étapes dans ton modèle puis définis toi-même leur ordre et leur durée en semaines."})]}),(0,jsx_runtime_1.jsx)("button",{className:"primary-button",onClick:create,children:"+ Nouveau modèle"})]}),
        (0,jsx_runtime_1.jsxs)("div",{className:"models-layout",children:[(0,jsx_runtime_1.jsxs)("aside",{className:"models-list panel",children:[(0,jsx_runtime_1.jsx)("strong",{children:"Mes modèles"}),...models.map(model=>(0,jsx_runtime_1.jsx)("button",{className:`model-list-item ${model.id===selectedId?'active':''}`,onClick:()=>setSelectedId(model.id),children:model.name},model.id)),!models.length&&(0,jsx_runtime_1.jsx)("p",{children:"Crée ton premier modèle."})]}),modelEditor]})
    ]});
}
function App() {
    ...
    const [models, setModels] = (0, react_1.useState)(() => loadPlanningModels());
    ...
    const [newProject, setNewProject] = (0, react_1.useState)({ ...emptyProject, modelId: '' });
    ...
    (0, react_1.useEffect)(() => {
        ...
        Promise.all([(0, repository_1.loadAppData)(), (0, repository_1.loadDismissedNotificationIds)()])
            ...
            if (!active)
                return;
            const uniqueProjects = Array.from(new Map(data.projects.map((project) => [project.id, project])).values());
            ...
            setSelectedProjectId(uniqueProjects.find((project) => !project.archivedAt)?.id ?? uniqueProjects[0]?.id);
        })
            .catch((reason) => {
    ...
    const createProject = async () => {
        ...
        const project = {
            ...
            updatedAt: new Date().toISOString(),
            stages: buildProjectStagesFromModel(newProject.modelId, models, newProject.startDate),
        };
        try {
            // saveProject() déclenche déjà la synchronisation locale et met à jour
            // l'état React. Ne pas ajouter le chantier une seconde fois ici.
            await (0, repository_1.saveProject)(project);
            setProjects((current) => {
                const exists = current.some((item) => item.id === project.id);
                return exists ? current.map((item) => item.id === project.id ? project : item) : [...current, project];
            ...
            setNewProject({ ...emptyProject, modelId: '' });
...
function CalendarView({ projects, events, onSaveEvent, onDeleteEvent }) {
    ...
    return ((0, jsx_runtime_1.jsxs)("div", { className: "view-stack calendar-view", children: [(0, jsx_runtime_1.jsxs)("header", { className: "page-header calendar-page-header", children: [(0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("span", { className: "eyebrow", children: "Rendez-vous et organisation" }), (0, jsx_runtime_1.jsx)("h1", { children: "Calendrier" }), (0, jsx_runtime_1.jsxs)("p", { children: ["Semaine ", (0, calendar_1.getIsoWeek)(weekStart), " \u00B7 ", (0, calendar_1.formatCalendarRange)(days[0], days[4])] })] }), (0, jsx_runtime_1.jsxs)("div", { className: "calendar-header-actions", children: [(0, jsx_runtime_1.jsx)("button", { className: "secondary-button", onClick: () => setWeekOffset((value) => value - 1), children: "\u2190 Pr\u00E9c\u00E9dente" }), (0, jsx_runtime_1.jsx)("button", { className: "secondary-button", onClick: () => setWeekOffset(0), children: "Aujourd\u2019hui" }), (0, jsx_runtime_1.jsx)("button", { className: "secondary-button", onClick: () => setWeekOffset((value) => value + 1), children: "Suivante \u2192" }), (0, jsx_runtime_1.jsx)("button", { className: "primary-button", onClick: openNewEvent, children: "+ Ajouter un \u00E9v\u00E9nement" })] })] }), (0, jsx_runtime_1.jsx)("section", { className: "calendar-panel panel", children: (0, jsx_runtime_1.jsx)("div", { className: "calendar-scroll", children: (0, jsx_runtime_1.jsxs)("div", { className: "calendar-week-grid", children: [(0, jsx_runtime_1.jsx)("div", { className: "calendar-corner", children: (0, jsx_runtime_1.jsx)("span", { children: "Heure" }) }), days.map((day) => {
                                ...
                                return ((0, jsx_runtime_1.jsxs)("div", { className: `calendar-day-column${dateId === todayId ? ' today' : ''}`, style: { height: `${calendarHeight}px` }, children: [hours.slice(0, -1).map((hour, index) => (0, jsx_runtime_1.jsx)("i", { className: "calendar-hour-line", style: { top: `${index * calendar_1.CALENDAR_HOUR_HEIGHT}px` } }, hour)), showCurrentTime && (0, jsx_runtime_1.jsx)("div", { className: "calendar-current-time", style: { top: `${currentTimeTop}px` }, children: (0, jsx_runtime_1.jsx)("span", {}) }), dayEvents.map((event, index) => {
                                            ...
                                            const sizeClass = geometry.height < 24 ? ' is-tiny' : geometry.height < 48 ? ' is-compact' : '';
                                            return ((0, jsx_runtime_1.jsxs)("button", { className: `calendar-event-card${sizeClass}`, style: { top: `${geometry.top}px`, height: `${geometry.height}px`, left: `${6 + (index % 2) * 3}px`, right: `${6 - (index % 2) * 3}px` }, onClick: () => openEvent(event), title: `${project?.name || 'Rendez-vous général'} · ${event.startTime} - ${event.endTime} · ${event.note}`, children: [(0, jsx_runtime_1.jsxs)("span", { children: [event.startTime, " - ", event.endTime] }), (0, jsx_runtime_1.jsx)("strong", { children: project?.name || 'Rendez-vous général' }), (0, jsx_runtime_1.jsx)("small", { children: event.note })] }, event.id));
                                        })] }, dateId));
...
function Dashboard({ projects, notifications, tasks, calendarEvents, onOpenPlanning, onOpenWeek, onOpenCalendar, onOpenTasks, onCompleteTask }) {
    ...
    return ((0, jsx_runtime_1.jsxs)("div", { className: "view-stack", children: [(0, jsx_runtime_1.jsxs)("header", { className: "page-header dashboard-home-header", children: [(0, jsx_runtime_1.jsx)("div", { className: "dashboard-home-heading", children: (0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("span", { className: "eyebrow", children: "Vue g\u00E9n\u00E9rale" }), (0, jsx_runtime_1.jsx)("h1", { children: "Tableau de bord" })] }) }), (0, jsx_runtime_1.jsxs)("div", { className: "dashboard-header-actions", children: [(0, jsx_runtime_1.jsxs)("button", { className: "week-number-pill", onClick: onOpenWeek, children: [(0, jsx_runtime_1.jsx)("span", { children: "Semaine actuelle" }), (0, jsx_runtime_1.jsx)("strong", { children: (0, calendar_1.getIsoWeek)() })] }), (0, jsx_runtime_1.jsx)("button", { className: "primary-button", onClick: onOpenPlanning, children: "Ouvrir le planning" })] })] }), (0, jsx_runtime_1.jsxs)("section", { className: "kpi-grid", children: [(0, jsx_runtime_1.jsxs)("article", { className: "kpi-card", children: [(0, jsx_runtime_1.jsx)("span", { children: "Chantiers actifs" }), (0, jsx_runtime_1.jsx)("strong", { children: projects.length }), (0, jsx_runtime_1.jsxs)("small", { children: [projects.filter((project) => !(0, planning_1.isProjectLate)(project)).length, " dans les temps"] })] }), (0, jsx_runtime_1.jsxs)("article", { className: "kpi-card task-soft", children: [(0, jsx_runtime_1.jsx)("span", { children: "T\u00E2ches \u00E0 faire" }), (0, jsx_runtime_1.jsx)("strong", { children: activeTasks.length }), (0, jsx_runtime_1.jsxs)("small", { children: [activeTasks.filter((task) => task.priority === 'urgent').length, " urgente(s)"] })] }), (0, jsx_runtime_1.jsxs)("article", { className: "kpi-card danger-soft", children: [(0, jsx_runtime_1.jsx)("span", { children: "Chantiers en retard" }), (0, jsx_runtime_1.jsx)("strong", { children: late }), (0, jsx_runtime_1.jsx)("small", { children: "\u00C0 traiter en priorit\u00E9" })] }), (0, jsx_runtime_1.jsxs)("article", { className: "kpi-card warning-soft", children: [(0, jsx_runtime_1.jsx)("span", { children: "Alertes \u00E0 venir" }), (0, jsx_runtime_1.jsx)("strong", { children: notifications.length }), (0, jsx_runtime_1.jsx)("small", { children: "Artisans \u00E0 pr\u00E9venir" })] })] }), (0, jsx_runtime_1.jsx)("section", { className: "dashboard-focus-grid dashboard-tasks-only", children: (0, jsx_runtime_1.jsxs)("section", { className: "panel dashboard-tasks-panel", children: [(0, jsx_runtime_1.jsxs)("div", { className: "panel-header", children: [(0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("span", { className: "eyebrow", children: "Prioritaire" }), (0, jsx_runtime_1.jsx)("h2", { children: "Mes t\u00E2ches" })] }), (0, jsx_runtime_1.jsx)("button", { className: "secondary-button", onClick: onOpenTasks, children: "G\u00E9rer toutes les t\u00E2ches" })] }), (0, jsx_runtime_1.jsxs)("div", { className: "dashboard-task-list", children: [!activeTasks.length && (0, jsx_runtime_1.jsxs)("button", { className: "dashboard-task-empty", onClick: onOpenTasks, children: [(0, jsx_runtime_1.jsx)("span", { children: "\u2713" }), (0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("strong", { children: "Aucune t\u00E2che en attente." }), (0, jsx_runtime_1.jsx)("small", { children: "Cliquer ici pour en ajouter une." })] })] }), activeTasks.slice(0, 6).map((task) => {
                                ...
                                }), activeTasks.length > 6 && (0, jsx_runtime_1.jsxs)("button", { className: "dashboard-task-more", onClick: onOpenTasks, children: ["+ ", activeTasks.length - 6, " autre(s) t\u00E2che(s)"] })] })] }) }), (0, jsx_runtime_1.jsxs)("section", { className: "dashboard-grid dashboard-agenda-grid", children: [(0, jsx_runtime_1.jsxs)("aside", { className: "panel dashboard-day-agenda dashboard-day-agenda-wide", children: [(0, jsx_runtime_1.jsxs)("header", { className: "dashboard-day-header", children: [(0, jsx_runtime_1.jsx)("div", { className: "dashboard-day-number", children: dayNumber }), (0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("span", { children: "Agenda du jour" }), (0, jsx_runtime_1.jsx)("strong", { children: dayName }), (0, jsx_runtime_1.jsxs)("small", { children: [monthName, " \u00B7 ", todayEvents.length, " rendez-vous"] })] }), (0, jsx_runtime_1.jsx)("button", { onClick: onOpenCalendar, "aria-label": "Ouvrir le calendrier", children: "\u2192" })] }), (0, jsx_runtime_1.jsxs)("div", { className: "dashboard-mini-calendar", onClick: onOpenCalendar, role: "button", tabIndex: 0, children: [(0, jsx_runtime_1.jsx)("div", { className: "dashboard-mini-time-axis", style: { height: `${miniCalendarHeight}px` }, children: miniHours.map((hour, index) => (0, jsx_runtime_1.jsxs)("span", { style: { top: `${index * calendar_1.MINI_CALENDAR_HOUR_HEIGHT}px` }, children: [String(hour).padStart(2, '0'), "h"] }, hour)) }), (0, jsx_runtime_1.jsxs)("div", { className: "dashboard-mini-day", style: { height: `${miniCalendarHeight}px` }, children: [miniHours.slice(0, -1).map((hour, index) => (0, jsx_runtime_1.jsx)("i", { style: { top: `${index * calendar_1.MINI_CALENDAR_HOUR_HEIGHT}px` } }, hour)), showCurrentTime && (0, jsx_runtime_1.jsx)("div", { className: "dashboard-current-time", style: { top: `${currentTimeTop}px` }, children: (0, jsx_runtime_1.jsx)("span", {}) }), todayEvents.map((event) => {
                                                const project = projects.find((item) => item.id === event.projectId);
                                                const geometry = (0, calendar_1.getCalendarEventGeometry)(event, calendar_1.MINI_CALENDAR_HOUR_HEIGHT);
                                                const sizeClass = geometry.height < 16 ? ' is-tiny' : geometry.height < 30 ? ' is-compact' : '';
                                                return (0, jsx_runtime_1.jsxs)("article", { className: `dashboard-mini-event${sizeClass}`, style: { top: `${geometry.top}px`, height: `${geometry.height}px` }, title: `${event.startTime} - ${event.endTime} · ${event.note}`, children: [(0, jsx_runtime_1.jsx)("strong", { children: project?.name || 'Rendez-vous général' }), (0, jsx_runtime_1.jsxs)("small", { children: [event.startTime, " - ", event.endTime, " \u00B7 ", event.note] })] }, event.id);
                                            }), !todayEvents.length && (0, jsx_runtime_1.jsxs)("div", { className: "dashboard-no-event", children: [(0, jsx_runtime_1.jsx)("span", { children: "Aucun rendez-vous aujourd\u2019hui" }), (0, jsx_runtime_1.jsx)("small", { children: "Cliquer pour ouvrir le calendrier" })] })] })] })] }), (0, jsx_runtime_1.jsxs)("aside", { className: "panel alert-panel", children: [(0, jsx_runtime_1.jsx)("div", { className: "panel-header", children: (0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("span", { className: "eyebrow", children: "\u00C0 faire" }), (0, jsx_runtime_1.jsx)("h2", { children: "Prochaines alertes" })] }) }), (0, jsx_runtime_1.jsxs)("div", { className: "compact-alerts", children: [notifications.slice(0, 6).map((notification) => (0, jsx_runtime_1.jsxs)("article", { className: `compact-alert ${notification.severity}`, children: [(0, jsx_runtime_1.jsx)("div", { className: "alert-date", children: (0, planning_1.formatShortDate)(notification.dueDate) }), (0, jsx_runtime_1.jsxs)("div", { children: [(0, jsx_runtime_1.jsx)("strong", { children: notification.projectName }), (0, jsx_runtime_1.jsx)("p", { children: notification.title })] })] }, notification.id)), !notifications.length && (0, jsx_runtime_1.jsx)("p", { className: "empty-state", children: "Aucune alerte imm\u00E9diate." })] })] })] })] }));
}
...
Cordialement,`;
    ...
    const planningWeeks = (0, react_1.useMemo)(() => {
        const dates = filtered.flatMap((project) => project.stages.map((stage) => stage.plannedDate).filter(Boolean)).sort();
        const toMonday = (value) => { const d=new Date(`${value}T12:00:00`); const day=(d.getDay()+6)%7; d.setDate(d.getDate()-day); return d; };
        const iso = (date) => { const d=new Date(Date.UTC(date.getFullYear(),date.getMonth(),date.getDate())); const day=d.getUTCDay()||7; d.setUTCDate(d.getUTCDate()+4-day); const yearStart=new Date(Date.UTC(d.getUTCFullYear(),0,1)); return Math.ceil((((d-yearStart)/86400000)+1)/7); };
        const base=dates.length?toMonday(dates[0]):toMonday(new Date().toISOString().slice(0,10));
        const end=dates.length?toMonday(dates[dates.length-1]):new Date(base); if(!dates.length) end.setDate(base.getDate()+77); end.setDate(end.getDate()+28);
        const result=[]; for(let d=new Date(base);d<=end;d.setDate(d.getDate()+7)){const key=d.toISOString().slice(0,10); result.push({key,number:iso(d),date:new Date(d)});} return result;
    },[filtered]);
    const weekKeyForDate = (value) => { if(!value)return ''; const d=new Date(`${value}T12:00:00`); const day=(d.getDay()+6)%7; d.setDate(d.getDate()-day); return d.toISOString().slice(0,10); };
...
function getCalendarEventGeometry(event, hourHeight = exports.CALENDAR_HOUR_HEIGHT) {
    ...
    const end = Math.min(exports.CALENDAR_END_HOUR * 60, Math.max(start + 1, timeToMinutes(event.endTime)));
    return {
